  ...
  <body>
    ...
    <div class="content">
      <div class="wrapp">
        ...
        <p class="marge">Email : <?php echo $data["email"]; ?> <a class="modif_info_bouton"><i class="fas fa-pencil-alt"></i> Modifier</a><i class="txt_modif_email cache"><form action="" method="post"><input type="text" name="email"/><input id="inscriBout" type="submit" value="Valider" onclick="modif()"/></form></i></p>
        <?php if(isset($_POST["email"])){
          $req = $bdd->query('UPDATE profil SET email=\''.$_POST["email"].'\' WHERE id=\''.$_GET["id"].'\'');
        } ?>
        <p class="marge">Numéro de telephone : <?php echo "+33".$data["numerotel"]; ?> <a class="modif_info_bouton2"><i class="fas fa-pencil-alt"></i> Modifier</a><i class="txt_modif_num cache"><form action="" method="post"><input type="tel" name="numerotel"/><input id="inscriBout" type="submit" value="Valider" onclick="modif()"/></form></i></p>
